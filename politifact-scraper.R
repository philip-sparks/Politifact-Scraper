...
url <- "http://www.politifact.com/truth-o-meter/statements/"
page_one_of <- str_extract_all(as(htmlParse(rawToChar(GET(url)$content)),"character"),"Page\\s+1\\s+of\\s+[[:digit:]]+")
total_pages <- as.numeric(str_extract(page_one_of,"[[:digit:]]+$"))

url2 <- "http://www.politifact.com/truth-o-meter/statements/?page="
statements <- list()
for(ii in 1:total_pages)
{
  cat("\r",ii)
  base.url <- paste0(rawToChar(GET(url2)$content),ii)
...
getFib <- function(urlpage)
{
  single.urlpage <- urlpage
  page <- htmlParse(rawToChar(GET(single.urlpage)$content))
